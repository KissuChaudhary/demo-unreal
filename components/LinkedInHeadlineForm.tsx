...
export default function LinkedInHeadlineForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/openai-api', {
        ...
        body: JSON.stringify({ ...formData, tool: 'linkedinHeadline' }),
      });
      if (!response.ok) {
  ...
  return (
    <div className="max-w-7xl mx-auto mt-10">
      <h1 className="text-4xl font-extrabold mb-8 text-center text-gray-800 tracking-tight">LinkedIn Headline Generator</h1>
      <p className="text-xl text-center text-gray-600 mb-12 max-w-3xl mx-auto">Create Compelling LinkedIn Headlines with Unrealshot AI – Professional, Concise, and Impactful.</p>
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/2">
          <div className="h-full p-6 border border-gray-200 rounded-xl shadow-sm">
            ...
            {generatedHeadline ? (
              <p className="text-gray-700 whitespace-pre-wrap text-lg leading-relaxed">{generatedHeadline}</p>
            ) : (
              <p className="text-gray-500 italic">Your generated LinkedIn headline will appear here.</p>
            )}
          </div>
        </div>
      </div>
