...
const StripePricingTable = ({ user }: Props) => {
  ...
  return (
    <>
      ...
      />
      <div className="grid grid-cols-1 px-4 py-4 gap-4 sm:grid-cols-3 md:gap-8">
        {Object.keys(plans).map((planKey) => {
          const plan = plans[planKey as PlanType];
          return (
            <div
              key={planKey}
              className="rounded-xl bg-white border border-gray-200 p-6 shadow-sm sm:px-8 lg:p-12"
            >
